...
namespace PockyBot.NET.Tests.Services
{
    public class TriggerResponseTesterTests
    {
        ...
        private readonly IPockyUserRepository _pockyUserRespository;
        ...
        public TriggerResponseTesterTests()
        {
            ...
            _pockyUserRespository = Substitute.For<IPockyUserRepository>();
            var options = Substitute.For<IOptions<PockyBotSettings>>();
            options.Value.Returns(_settings);
            _subject = new TriggerResponseTester(options, _pockyUserRespository);
        }

        [Theory]
        [MemberData(nameof(TriggerResponseTesterTestData.RoomTestData), MemberType = typeof(TriggerResponseTesterTestData))]
        internal void TestShouldTriggerInRoom(PockyBotSettings settings, PockyUser user, Message message, ITrigger trigger, bool shouldTrigger)
        {
        ...
        [Theory]
        [MemberData(nameof(TriggerResponseTesterTestData.DirectMessageTestData), MemberType = typeof(TriggerResponseTesterTestData))]
        internal void TestShouldTriggerInDirectMessage(PockyUser user, Message message, ITrigger trigger, bool shouldTrigger)
        {
        ...
        {
            _pockyUserRespository.GetUser(user.UserId).Returns(user);
        }
